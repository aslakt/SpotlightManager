...
Function Import-SpotlightPictures {
    ...
    $Hashes = (Get-ChildItem $Destination | Get-FileHash)
    $counter = 0
            ...
            If ($Hashes.Hash -ne $null) {
                If (!($Hashes.Hash.Contains( (Get-FileHash $_.Path).Hash ) )) {
                    If($counter -lt 10) {
                        ...
                        Copy-Item $_.Path -Destination "$Destination\Spotlight0$counter.jpg"
                        $counter++
                    } Else {
                        While (Test-Path "$Destination\Spotlight$counter.jpg") {
                            $counter++
                        }
                        Copy-Item $_.Path -Destination "$Destination\Spotlight$counter.jpg"
                        $counter++
                    }
                }
            } Else {
